---@class Minisnacks: minisnacks.plugins
local M = {}

setmetatable(M, {
  __index = function(t, k)
    ---@diagnostic disable-next-line: no-unknown
    t[k] = require("minisnacks." .. k)
    return rawget(t, k)
  end,
})

_G.Minisnacks = M
_G.svim = vim.fn.has("nvim-0.11") == 1 and vim or require("minisnacks.compat")

M.version = "2.30.0" -- x-release-please-version

---@class minisnacks.Config.base
---@field example? string
---@field config? fun(opts: table, defaults: table)

---@class minisnacks.Config: minisnacks.plugins.Config
---@field styles? table<string, minisnacks.win.Config>
local config = {}
config.styles = {}

---@class minisnacks.config: minisnacks.Config
...
function M.config.get(snack, defaults, ...)
  ...
  for i = 1, select("#", ...) + 2 do
    local v = todo[i] --[[@as minisnacks.Config.base]]
...
---@param name string
---@param defaults minisnacks.win.Config|{}
---@return string
...
---@param opts minisnacks.Config?
function M.setup(opts)
  if M.did_setup then
    return vim.notify("minisnacks.nvim is already setup", vim.log.levels.ERROR, { title = "minisnacks.nvim" })
  end
  M.did_setup = true

  if vim.fn.has("nvim-0.9.4") ~= 1 then
    return vim.notify("minisnacks.nvim requires Neovim >= 0.9.4", vim.log.levels.ERROR, { title = "minisnacks.nvim" })
  end
  ...
  local group = vim.api.nvim_create_augroup("minisnacks", { clear = true })
