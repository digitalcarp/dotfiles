...
M.meta = {
  desc = "Utility functions for Minisnacks _(library)_",
}
...
---@alias minisnacks.util.hl table<string, string|vim.api.keyset.highlight>

local hl_groups = {} ---@type table<string, vim.api.keyset.highlight>
vim.api.nvim_create_autocmd("ColorScheme", {
  group = vim.api.nvim_create_augroup("minisnacks_util_hl", { clear = true }),
  callback = function()
...
---@param groups minisnacks.util.hl
...
function M.winhl(...)
  ...
  return Minisnacks.config.merge(unpack(ret))
end
...
function M.is_transparent()
  if transparent == nil then
    ...
    vim.api.nvim_create_autocmd("ColorScheme", {
      group = vim.api.nvim_create_augroup("minisnacks_util_transparent", { clear = true }),
      callback = function()
        transparent = nil
      end,
    })
